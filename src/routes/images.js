...
function createImageRoutes(config, idManager, upstreamManager) {
  ...
  // GET /emby/Items/:itemId/Images/:imageType/:imageIndex?
  // Some clients prefix with /emby
  router.get('/emby/Items/:itemId/Images/:imageType/:imageIndex?', async (req, res) => {
    try {
      const resolved = req.resolveId(req.params.itemId);
      if (!resolved) {
        return res.status(404).end();
      ...
      logger.error(`Error proxying emby image: ${err.message}`);
      if (!res.headersSent) res.status(502).end();
    }
  });
